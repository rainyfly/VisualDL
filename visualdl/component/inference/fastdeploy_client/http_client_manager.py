...
class HttpClientManager:
    ...
    def infer(self, server_url, model_name, model_version, inputs):
        ...
        inputs, outputs = self.prepare_request(input_metadata, inputs,
                                               output_metadata)
    ...
    def prepare_request(self, inputs_meta, inputs_data, outputs_meta):
        '''
        ...
        for input_dict in inputs_meta:
            ...
            if input_name not in inputs_data:
                raise RuntimeError(
                    'Error: input name {} required for model not existed.'.
                    format(input_name))
            ...
            if len(input_dict['shape']
                ...
                inputs_data[input_name] = inputs_data[input_name].transpose(
                    0, 3, 1, 2)
            infer_input = httpclient.InferInput(input_name,
                                                inputs_data[input_name].shape,
                                                input_dict['datatype'])
...
class GrpcClientManager:
    ...
    def _create_client(self, server_url):
        if server_url in self.clients:
            return self.clients[server_url]
        try:
            fastdeploy_client = grpcclient.InferenceServerClient(server_url)
            self.clients[server_url] = fastdeploy_client
            return fastdeploy_client
        except Exception:
            raise RuntimeError(
    ...
    def prepare_request(self, inputs_meta, inputs_data, outputs_meta):
        ...
        outputs = []
        for output_dict in outputs_meta:
            infer_output = grpcclient.InferRequestedOutput(output_dict.name)
            outputs.append(infer_output)
        return inputs, outputs
